...
window.addEventListener('scroll', () => {
    ...
    navLinks.forEach(link => {
        link.classList.remove('is-active', 'bg-slate-900', 'text-white', 'dark:bg-white', 'dark:text-black', 'shadow-lg', 'shadow-primary/20', 'font-bold');
        link.classList.add('text-slate-600', 'dark:text-slate-400');
        if (link.getAttribute('href').includes(current)) {
            link.classList.add('is-active', 'bg-slate-900', 'text-white', 'dark:bg-white', 'dark:text-black', 'shadow-lg', 'shadow-primary/20', 'font-bold');
...
contactForm.addEventListener('submit', (e) => {
    ...
    btn.innerHTML = '<i class="fas fa-circle-notch animate-spin"></i> Envoi...';
    btn.disabled = true;
    ...
    setTimeout(() => {
        ...
        const subject = encodeURIComponent(`Nouveau contact portfolio - ${name}`);
        const body = encodeURIComponent(`Nom: ${name}\nEmail: ${email}\n\nMessage:\n${message}`);
        window.location.href = `mailto:tolofonnelkael@gmail.com?subject=${subject}&body=${body}`;
        showToast('Votre message est pret dans votre application email.');
        e.target.reset();
